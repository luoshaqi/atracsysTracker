...
# while True:    
    ...
    # marker_data = np.zeros((5,3))
    found_marker_id = -1
    found_marker_index = -1
    found_marker_data = (0,0,0)
    found_marker_rotation = [(0,0,0),(0,0,0),(0,0,0)]
    # found_marker_rotation = np.zeros((3,3))
    for marker in frame.markers:
        ...
        if markers_indexes[marker.geometry_id] < max_number_of_tracked_markers:
            ...
            # marker_data_r[markers_indexes[marker.geometry_id]] = (
            #     marker.position[0], marker.position[1], marker.position[2])
            ...
            if all_zero == False:
                ...
                found_marker_data = marker.position
                found_marker_rotation = marker.rotation
    ...
    data = {'timestamp':current_time,
            ...
            'CoordX':[found_marker_data[0]],
            'CoordY':[found_marker_data[1]],
            'CoordZ':[found_marker_data[2]],
            'R00':[found_marker_rotation[0][0]],
            'R01':[found_marker_rotation[0][1]],
            'R02':[found_marker_rotation[0][2]],
            'R10':[found_marker_rotation[1][0]],
            'R11':[found_marker_rotation[1][1]],
            'R12':[found_marker_rotation[1][2]],
            'R20':[found_marker_rotation[2][0]],
            'R21':[found_marker_rotation[2][1]],
            'R22':[found_marker_rotation[2][2]],}
...
# df.to_pickle('~/Atracsys/fusionTrack_SDK-v4.7.4-linux64/python/scripts/data14.pkl')
# df.to_csv('~/Atracsys/fusionTrack_SDK-v4.7.4-linux64/python/scripts/data15.csv',index=False)
df.to_csv(save_path, index=False)


# df = pd.read_pickle('~/Desktop/tmp.pkl')
